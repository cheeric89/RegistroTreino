...
export default function WorkoutSummary({ workout, onDone, onRepeat, onOpenHistory }) {
  ...
  const summary = useMemo(() => {
    ...
    const exerciseCategories = Array.isArray(workout.exercises)
      ? workout.exercises
      : [];
      ...
      .forEach((item) => {
        (item?.exercises || []).forEach((category) => {
          (category?.exercises || []).forEach((exercise) => {
            ...
            if (!existing || isBetterStrengthSet(best, existing)) {
              previousBestByExercise.set(key, best);
            }
          });
        });
      });
    ...
    const categories = exerciseCategories.map((category) => {
      ...
      const exercises = (category?.exercises || []).map((exercise) => {
        ...
        const poolForBest = sets.some((set) => set?.done)
        ...
        if (currentBestVolume) {
          ...
          if (!bestSessionSet) {
            ...
            if (
              ...
              (candidateVolume === bestVolume && isBetterStrengthSet(candidate, bestSessionSet))
            ) {
              bestSessionSet = candidate;
            }
          }
    ...
    const totalVolume = Number.isFinite(storedVolume) && storedVolume > 0
      ? storedVolume
      : calculatedVolume;
  ...
  const bestSetValues = summary.bestSessionSet
    ? getSetValues(summary.bestSessionSet)
    : null;
  ...
  return (
    <div className="screen summary-screen summary-v2 flow-screen">
      <div className="summary-v2__content">
        <header className="summary-v2__hero">
          <div className="summary-v2__success-ring" aria-hidden="true">
            <CheckCircle2 size={42} />
          </div>
          <span className="page-eyebrow">Entrenamiento guardado</span>
          <h1>¡Sesión completada!</h1>
          <p className="summary-v2__date">
            {workout.day ?? "Entrenamiento"} · {workout.date ?? ""}
          </p>
          <p className="summary-v2__hero-copy">
            Buen trabajo. Tu sesión quedó registrada y ya forma parte de tu progreso.
          </p>
        </header>

        <section className="summary-v2__metrics" aria-label="Resumen del entrenamiento">
          <article className="summary-v2__metric-card">
            <span className="summary-v2__metric-icon"><Target size={18} /></span>
            <strong>{summary.completedSets}</strong>
            <span>Series</span>
            <small>de {summary.totalSets} planificadas</small>
          </article>

          <article className="summary-v2__metric-card">
            <span className="summary-v2__metric-icon"><Dumbbell size={18} /></span>
            <strong>{summary.totalExercises}</strong>
            <span>Ejercicios</span>
            <small>{summary.totalMuscles} {summary.totalMuscles === 1 ? "músculo" : "músculos"}</small>
          </article>

          <article className="summary-v2__metric-card">
            <span className="summary-v2__metric-icon"><BarChart2 size={18} /></span>
            <strong>{summary.totalVolume > 0 ? formatNumber(summary.totalVolume) : "—"}</strong>
            <span>Volumen</span>
            <small>kg acumulados</small>
          </article>

          <article className="summary-v2__metric-card">
            <span className="summary-v2__metric-icon"><Clock3 size={18} /></span>
            <strong>{formatDuration(workout.duration)}</strong>
            <span>Duración</span>
            <small>tiempo entrenado</small>
          </article>
        </section>

        <section className="summary-v2__highlights">
          <div className="summary-v2__section-heading">
            <div>
              <span className="card-kicker">Resumen inteligente</span>
              <h2>Lo mejor de hoy</h2>
            </div>
            <Sparkles size={20} />
          </div>

          <div className="summary-v2__highlight-grid">
            <article className="summary-v2__highlight-card summary-v2__highlight-card--pr">
              <Trophy size={19} />
              <strong>{summary.prCount}</strong>
              <span>Récords personales</span>
              <small>{prSupportingText}</small>
            </article>

            <article className="summary-v2__highlight-card">
              <Dumbbell size={19} />
              <strong>
                {bestSetValues
                  ? `${bestSetValues.reps} × ${bestSetValues.weight} kg`
                  : "—"}
              </strong>
              <span>Mejor serie</span>
              <small>{summary.bestSessionSet?.exerciseName || "Sin datos suficientes"}</small>
            </article>
            ...
            <article className="summary-v2__highlight-card">
              ...
              <small>{summary.completedSets}/{summary.totalSets} series completadas</small>
            </article>
          </div>
        </section>

        <section className="summary-v2__training-card">
          <div className="summary-v2__section-heading summary-v2__section-heading--training">
            <div>
              <span className="card-kicker">Detalle de la sesión</span>
              <h2>{summary.totalExercises} ejercicio{summary.totalExercises !== 1 ? "s" : ""}</h2>
            </div>
            <BarChart2 size={20} />
          </div>
          ...
          <div className="summary-v2__categories">
            {summary.categories.map((category, categoryIndex) => (
              <section key={`${category?.name}-${categoryIndex}`} className="summary-v2__category">
                <header className="summary-v2__category-header">
                  <div>
                    <span className="cat-dot cat-dot--sm" />
                    <strong>{category?.name ?? "Sin nombre"}</strong>
                  </div>
                  <span>
                    {category.categoryCompleted}/{category.categorySets} series · {formatNumber(category.categoryVolume)} kg
                  </span>
                </header>
                ...
                <div className="summary-v2__exercise-list">
                  {(category?.exercises ?? []).map((exercise, exerciseIndex) => (
                    <article
                      ...
                      className="summary-v2__exercise"
                    >
                      <div className="summary-v2__exercise-heading">
                        <div className="summary-v2__exercise-title">
                          <strong>{exercise?.name ?? "Ejercicio"}</strong>
                          {exercise.isPR && <span className="summary-v2__pr-badge">PR</span>}
                          {!exercise.isPR && exercise.isFirstReference && (
                            <span className="summary-v2__first-badge">Nueva marca</span>
                          )}
                        </div>
                      ...
                      <div className="summary-v2__set-list">
                        {exercise.sets.map((set, setIndex) => {
                          ...
                          const stateClass = set?.done
                            ? "summary-v2__set-chip--done"
                            : hasData
                              ? "summary-v2__set-chip--pending"
                              : "summary-v2__set-chip--empty";

                          return (
                            <span
                              key={`${setIndex}-${set?.weight}-${set?.reps}`}
                              className={`summary-v2__set-chip ${stateClass}`}
                              title={set?.done ? "Serie completada" : hasData ? "Serie sin confirmar" : "Serie no realizada"}
                            >
                              {formatSet(set)}
                            </span>
                          );
          ...
          <div className="summary-v2__legend" aria-label="Leyenda de series">
            <span><i className="summary-v2__legend-dot summary-v2__legend-dot--done" /> Completada</span>
            <span><i className="summary-v2__legend-dot summary-v2__legend-dot--pending" /> Sin confirmar</span>
            <span><i className="summary-v2__legend-dot summary-v2__legend-dot--empty" /> No realizada</span>
          </div>
        </section>

        <section className="summary-v2__actions">
          <div>
            <span className="card-kicker">Siguiente paso</span>
            <h2>¿Qué quieres hacer ahora?</h2>
          </div>

          <div className="summary-v2__secondary-actions">
            {onRepeat && (
              <button type="button" onClick={() => onRepeat(workout)}>
                <RotateCcw size={17} />
                Repetir sesión
              </button>
            )}
            {onOpenHistory && (
              <button type="button" onClick={() => onOpenHistory(getWorkoutGroup(workout))}>
                <BarChart2 size={17} />
                Ver historial
              </button>
            )}
